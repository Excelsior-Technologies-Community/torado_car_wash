...
import { vehicleCategoriesApi, servicesApi, washPackagesApi, bookingsApi } from '../api'
...
function Book() {
    ...
    const fetchWashPackages = async () => {
        try {
            ...
            const data = Array.isArray(response) ? response : (response.data || [])
            console.log('Wash packages:', data)
    ...
    const handleBooking = async () => {
        ...
        const errors = []
        if (!selectedCategory) errors.push('Vehicle Type')
        if (selectedServices.length === 0) errors.push('Services')
        if (!selectedPackage) errors.push('Wash Package')
        if (!selectedDate) errors.push('Date')
        if (!selectedTime) errors.push('Time')
            ...
            <div className="min-h-screen bg-gray-50 py-12">
                ...
                <div className="bg-white rounded-lg shadow-md p-8 mb-8 ">
                    ...
                    <Swiper
                        ...
                        slidesPerView={1}
                        breakpoints={{
                            640: { slidesPerView: Math.max(1, Math.min(2, categories.length || 1)) },
                            1024: { slidesPerView: Math.max(1, Math.min(3, categories.length || 1)) }
                        }}
                    ...
                    >
                        {categories && categories.length > 0 && categories.map((category, index) => (
                            <SwiperSlide key={category.id || index} className="h-full">
                                {({ isActive }) => (
                                    <div className="flex flex-col items-center h-full">
                                        <div
                                            className={`w-48 h-48 rounded-full flex items-center justify-center transition-all duration-300 ${isActive ? 'bg-yellow-400 scale-100' : 'bg-gray-100 scale-90 opacity-60'
                                                }`}
                                        >
                                            {category.image && (
                                                <img
                                                    src={`${uploadBaseUrl}/uploads/${category.image}`}
                                                    alt={category.name}
                                        ...
                                        <h3 className={`mt-6 text-xl font-semibold transition-all ${isActive ? 'text-black' : 'text-gray-400'
                                            }`}>
                                            {category.name}
                                        </h3>
                                    </div>
                ...
                {selectedCategory && (
                    ...
                    <>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                                    {services.map((service) => (
                                        <div
                                            ...
                                            className={`cursor-pointer rounded-lg p-6 transition-all ${selectedServices.includes(service.id)
                                        ...
                                        >
                                            {service.image && (
                                                <img
                                                    src={`${uploadBaseUrl}/uploads/${service.image}`}
                                                    alt={service.name}
                        ...
                        <div className="bg-white rounded-lg shadow-md p-8 mb-8">
                            ...
                            {washPackages && washPackages.length > 0 ? (
                                <div className={`grid gap-4 sm:gap-6 ${washPackages.length === 1 ? 'grid-cols-1 max-w-md mx-auto' :
                                    washPackages.length === 2 ? 'grid-cols-1 sm:grid-cols-2' :
                                        washPackages.length === 3 ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' :
                                    ...
                                    {washPackages.map((pkg, index) => (
                                        <div
                                            ...
                                            className={`cursor-pointer rounded-lg overflow-hidden transition-all ${selectedPackage === pkg.id
                                                ? 'scale-110 shadow-2xl ring-4 ring-yellow-400 z-10'
                                                : 'shadow-lg hover:shadow-xl'
                                                }`}
                                        >
                                            <div className={`text-white text-center py-8 relative ${selectedPackage === pkg.id
                                                ? 'bg-yellow-500'
                            ...
                            <>
                                <div className="bg-white rounded-lg shadow-md p-8 mb-8">
                                    ...
                                    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                                        <div className="grid grid-cols-3 bg-gray-50 p-3 sm:p-4 font-semibold text-xs sm:text-sm text-center">
                                            <div>Days</div>
                                        ...
                                        {[
                                            ...
                                            <div key={idx} className={`grid grid-cols-3 p-3 sm:p-4 text-center text-xs sm:text-sm border-t ${schedule.bg}`}>
                                                        ...
                                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                                            {availableSlots.map((slot, index) => (
                                                                <button
                                                                    ...
                                                                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${selectedTime === slot.time_slot
                                        ...
                                        {selectedDate && selectedTime && (
                                            <>
                                                ...
                                                <button
                                                    ...
                                                    disabled={isBooking}
                                                    className="mt-6 w-full bg-orange-500 text-white font-semibold py-3 rounded-lg hover:bg-orange-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                                >
