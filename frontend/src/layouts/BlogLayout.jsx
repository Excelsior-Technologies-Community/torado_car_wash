    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      {hasActiveFilters && (
        <div className="mb-6 flex flex-wrap gap-2 items-center">
          ...
          {filters.tags && filters.tags.length > 0 && filters.tags.map(tagSlug => (
          ...
          <button 
            ...
            className="ml-auto px-4 py-1 rounded border border-gray-300 bg-white hover:bg-orange-50 hover:border-orange-500 hover:text-orange-500 transition text-sm"
          >
            Clear All
          </button>
        </div>
      ...
      <div className={`flex flex-col lg:flex-row gap-8 ${layout === 'right-sidebar' ? 'lg:flex-row-reverse' : ''}`}>
        {layout !== 'grid' && (
          <BlogSidebar
        ...
        <div className="flex-1">
          <div className={`grid gap-8 mb-10 ${layout === 'grid' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1 md:grid-cols-2'}`}>
            {blogs && blogs.length > 0 && blogs.map(blog => (
              <BlogCard key={blog.id} blog={blog} />
            ))}
          </div>

          {blogs && blogs.length === 0 && <div className="text-center py-20 text-gray-500">No blogs found</div>}
